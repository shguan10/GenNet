  ...
  for (data, target) in tqdm((train_loader)):
    ...
    for bind,pi in enumerate(chosen):
      ...
      for l in range(model.numlayers):
        lgs[l*2] += 2 * model.layers[l].bias.grad.t() @ model.layers[l].bias.grad @ model.layers[l].weight
      zerograd(model)
...
def main():
  ...
  for epoch in range(1, args.epochs + 1):
    if args.large_margin:
      train_FCN(args, model, device, train_loader, optimizer, epoch)
    else: 
